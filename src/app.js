...
async function startGame() {
    ...
    const player = await Player.create(new THREE.Vector3(0, 15, 0), physic, './src/entity/player/tung/skin.fbx', './src/entity/player/tung/');

    // Tworzenie wrogów
    const enemies = [];
    for (const path of patrolGroups) {
        const enemy = await Enemy.create(path, physic,
            './src/entity/player/player/skin.fbx',       // Model
            './src/entity/player/player/'                // Folder z animacjami
        );
